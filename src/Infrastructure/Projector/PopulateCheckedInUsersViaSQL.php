...
final class PopulateCheckedInUsersViaSQL
{
    ...
    public function __invoke()
    {
        ...
        $sql = <<<'SQL'
SELECT event_name, aggregate_id, payload
FROM event_stream
WHERE event_name IN(?, ?, ?)
...
SQL;
        ...
        $statement = $this->connection->executeQuery($sql, [
            ...
            UserCheckedOutFromBuilding::class,
        ]);
        ...
        while ($row = $statement->fetch(\PDO::FETCH_ASSOC)) {
            ...
            if ($row['event_name'] === UserCheckedOutFromBuilding::class) {
        ...
        foreach ($usersInBuildings as $buildingId => $users) {
            file_put_contents(__DIR__."/../../../public/sql-$buildingId.json", json_encode($users));
        }
    }
}
